  ...
  src={
    item.image
      ? item.image.includes("http")
        ? item.image
        : `https://mintlify.s3.us-west-1.amazonaws.com/ashton17h/static/img/gallery/${item.image}`
      : `https://mintlify.s3.us-west-1.amazonaws.com/ashton17h/static/img/gallery/default.png`
  }
  style={{
    height: 150,
    width: "fit-content",
...
const colab_href = `https://colab.research.google.com/github/ashton17h/os1/blob/main/${item.source}`;
const github_href = `https://github.com/ashton17h/os1/blob/main/${item.source}`;
...
<div className="examples-gallery-container">
  <select multiple className="tag-filter" data-placeholder="Filter by tags">
    {allTags.map((tag) => (
