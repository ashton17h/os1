...
- Build a real-time voice application using [WebRTC](https://webrtc.org/) and connect it with the [**`RealtimeAgent`**](/docs/reference/agentchat/realtime_agent/realtime_agent). [Demo implementation](https://github.com/ashton17h/realtime-agent-over-webrtc).
...
<Note>
  ...
  [here](https://github.com/ashton17h/realtime-agent-over-webrtc/tree/main).
</Note>
...
```bash
git clone https://github.com/ashton17h/realtime-agent-over-webrtc.git
...
/WebRTC.js](https://github.com/ashton17h/realtime-agent-over-webrtc/blob/main/realtime_over_webrtc/website_files/static/WebRTC.js), so lets take a look at the code there first.
...
async def index_page():
    return {"message": "WebRTC os1 Server is running!"}
```
