...
class PatchObject(ABC):
    ...
    def msg(self) -> str:
        ...
        return f"Module{'s' if plural else ''} {modules_str} needed for {fqn} {'are' if plural else 'is'} missing, please install it using 'pip install os1[{self.dep_target}]'"
...
def require_optional_import(modules: Union[str, Iterable[str]], dep_target: str) -> Callable[[T], T]:
    ...
    Args:
        ...
        dep_target: Target name for pip installation (e.g. 'test' in pip install os1[test])
    """
